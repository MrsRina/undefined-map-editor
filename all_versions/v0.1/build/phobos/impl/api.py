from .lib import pygame, GL11, math, os, Flag, json;
from .    import util;
	...
	def render(self):
		if self.entity_in is not None:
			if self.motion == 0 or motion == 3:
				self.entity_in.left = -1 if self.left else 0;
				self.entity_in.right = 1 if self.right else 0;
				self.entity_in.up = 1 if self.up else 0;
				self.entity_in.down = -1 if self.down else 0;
...
class GameGui:
	...
	def registry(self, gui):
		self.guis[gui.tag] = gui;

	def get(self, gui_tag):
		if self.guis.__contains__(gui_tag):
			return self.guis[gui_tag];
