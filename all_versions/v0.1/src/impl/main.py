...
class Game:
	...
	def init(self):
		...
		self.game_gui.registry(ingame.StartGameGui(self));
		self.game_gui.registry(ingame.Loading(self));
		self.game_gui.registry(ingame.MainMenu(self));
	...
	def run(self):
		while 1:
			...
			if self.update_stamp.is_passed(16.666):
	...
	def handler_out_event(self):
		...
		if self.stage_start == 0 and type(self.game_gui.current_gui) != ingame.StartGameGui and self.current_map is None:
			self.game_gui.open("StartGameGui");
		...
		if self.loading_now == "refresh":
			if self.loading_stamp.is_passed(self.loading_time):
				...
				if self.loaded is False and self.file is not "":
					self.map_editor.new_map("random", -1);
					self.map_editor.init();

					self.current_map.load(self.file);
