...
class Editor:
	...
	def new_map(self, tag, id):
		...
		map_ = universe.Map(self.master, tag, id);
		map_.edit_mode = True;
		map_.grid = True;
	...
	def render(self, partial_ticks):
		if self.interface is not None and self.operable:
			...
			self.master.font_renderer.render("XY " + "[" + str(self.master.camera.x) + ", " + str(self.master.camera.y) + "] " + "Mapa: " + self.current_map_tag + " Img. Totais: " + str(self.totally_images), 1, self.master.screen_rect.h - self.master.font_renderer.get_height() - 1, [255, 255, 255, 255]);