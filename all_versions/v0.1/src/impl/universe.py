...
#
# IMPORTANTE:
# Eu tinha visto um tutorial como fazer a pora de um tile game,
...
class Object:
	...
	def update(self, partial_ticks, texture_manager, camera):
		...
		if self.found_texture is None or self.found_texture.tag is not self.get_name():
			...
			if self.found_texture.image is None:
				return;
...
class Map:
	def __init__(self, master, tag, id):
		self.master = master;
	...
	def update(self, partial_ticks):
		...
		if self.edit_mode is False:
			...
			self.rect.h = self.master.screen_rect.h;
			...
			self.master.camera.last_tick_x = -self.master.player.rect.x + (self.rect.w / 2);
			self.master.camera.last_tick_y = -self.master.player.rect.y + (self.rect.h / 2);
